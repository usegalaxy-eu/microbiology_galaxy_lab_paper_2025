...
save_plot(multi, "../docs/extended/supplementary_figure_2_v1", 12, 12)
# save_plot(c_2, "../docs/extended/supplementary_figure_2_v2", 18, 6)
```
...
a <- plot_dt |>
    ...
    theme(
        ...
        axis.title.x = element_markdown(margin = margin(t = 10), size = 12),
...
# reorder <- plot_dt_2[which(cluster != "Other"), by = .(cluster, variable), .(value = value |> median())]
# reorder <- reorder[order(-value)]
# 
# plot_dt_2$cluster <- plot_dt_2$cluster |> factor(levels = c(reorder$cluster |> unique(), "Other"))
...
b <- plot_dt_2 |>
    ...
    geom_point(fill = "grey", color = "grey10", shape = 21, size = 1.5, stroke = .1, 
...
d1_p <- ggplot() +
    ...
    theme(
        ...
        axis.title.x = element_markdown(margin = margin(t = 10), face = "bold", size = 12),
        axis.title.y = element_markdown(margin = margin(r = 10), size = 12),
...
multi <- ((d1_p | a) / b) + 
    ...
    theme(
        plot.tag = element_text(face = "bold", size = 18, family = "Calibri"),
...
save_plot(multi, "../docs/extended/figure_2", 10, 12)
